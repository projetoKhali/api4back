...
public class PartnerService implements IPartnerService {
  ...
  @Autowired ExpertiseRepository expertiseRepository;

  @Autowired QualifierRepository qualifierRepository;

  @Autowired ExpertiseQualifierRepository expertiseQualifierRepository;

  @Autowired PartnerTrackRepository partnerTrackRepository;

  @Autowired PartnerExpertiseRepository partnerExpertiseRepository;

  @Autowired PartnerQualifierRepository partnerQualifierRepository;

  @Autowired ExpertiseService expertiseService;

  @Autowired QualifierService qualifierService;
    ...
    for (PartnerTrack partnerTrack : partnerTracks) {
      ...
      for (PartnerExpertise partnerExpertise : partnerExpertises) {
        ...
        if (expertise.getTrack().getName().equals(partnerTrackDTO.getName())) {
          ...
          List<PartnerQualifier> partnerQualifiers =
              partnerQualifierRepository.findByPartner(partner);
          ...
          for (PartnerQualifier partnerQualifier : partnerQualifiers) {
            ...
            boolean isRelated =
                expertiseQualifierRepository.existsByExpertiseIdAndQualifierId(
                    expertise, qualifier);
    ...
    for (PartnerTrack partnerTrack : partnersTracks) {
      ...
      ProductPartnerDTO partnerDTO =
          new ProductPartnerDTO(
              partnerOptional.get(),
              partnerTrack,
              expertiseService.findExpertisesDTOByPartnerAndTrack(
                  track.get(), partnerOptional.get()),
              qualifierService.findQualifiersDTOByPartnerAndTrack(
                  track.get(), partnerOptional.get()));
